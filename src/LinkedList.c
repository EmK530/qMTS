...
LinkedList *LinkedList_create() {
  LinkedList *newList = (LinkedList*)wcalloc(1, sizeof(LinkedList)); /* zeroed allocation, just what we need */
  if (!newList) {
    print("[LinkedList] Allocation error in LinkedList_create\n");
    return NULL;
  }
  return newList;
}
...
int LinkedList_append(LinkedList *list, void *item) {
  if (list == NULL)
    return 1;
  Node *newNode = (Node*)wcalloc(1, sizeof(Node));
  if (newNode == NULL)
    return 1;
